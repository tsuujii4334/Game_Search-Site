...
review2 = Review.find_or_create_by!(user_id: yamada.id, game_id: game1.id) do |review|
  review.review_writing = "訂正：10~15時間ほどでクリアできる丁度いいボリュームのゲームです。"
end

review3 = Review.find_or_create_by!(user_id: yamada.id, game_id: game1.id) do |review|
  review.review_writing = "追記：どこでもセーブできてやり直せるので慣れてない人でも遊びやすいです。"
end

Comment.find_or_create_by!(user_id: kimura, review_id: review2.id) do |comment|
  comment_writing = "なるほど参考になります＠＠＠"
end

Comment.find_or_create_by!(user_id: kimura, review_id: review2.id) do |comment|
  comment_writing = "訂正：なるほど参考になります。"
end
